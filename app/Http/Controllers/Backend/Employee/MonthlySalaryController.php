...
class MonthlySalaryController extends Controller
{
    ...
    public function getSalary(Request $request)
    {
        ...
        foreach ( $data as $key => $attendance){
            $total_attendance = EmployeeAttendance::with(['user'])->where($where)->where('employee_id',$attendance->employee_id)->get();
            $absentcount = count($total_attendance->where('attendance_status','Absent'));
            $color = 'success';
            $html[$key]['tdsource'] = '<td>'.($key+1).'</td>';
            $html[$key]['tdsource'] .= '<td>'.$attendance['user']['id_no'].'</td>';
            ...
            $html[$key]['tdsource'] .= '<td>'.(int)$totalsalary.'</td>';
            $html[$key]['tdsource'] .= '<td>';
            $html[$key]['tdsource'] .= '<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blank" href="'.route("employees.monthly.salary.payslip",$attendance->employee_id).'">Pay Slip</a>';
            $html[$key]['tdsource'] .= '</td>';
        }
