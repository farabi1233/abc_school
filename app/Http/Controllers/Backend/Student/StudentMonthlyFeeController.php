...
class StudentMonthlyFeeController extends Controller
{
    ...
    public function getStudent(Request $request)
    {
      ...
      foreach ( $allStudent as $key => $v){
          $registrationFee = FeeCategoryAmmount::where('fee_category_id','2')-> where('class_id', $v->class_id)->first();
          $color = 'success';
          ...
          $html[$key]['tdsource'] .= '<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blank" href="'.route("students.monthly_fee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.'&month='.$request->month.'">Fee Slip</a>';
          $html[$key]['tdsource'] .= '</td>';
    ...
    public function paySlip(Request $request)
    {
        ...
        $pdf = PDF::loadView('backend.student.monthly_fee.fee_slip',$data);
        return $pdf->stream('monthly-fee-payslip-'.$data['details']['student']['id_no'].'.pdf');
